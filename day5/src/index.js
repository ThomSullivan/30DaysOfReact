...
function Title() {
  useEffect(() => {
    document.title = 'Day 5';
  }, []);
}
const NameElement = (props) =>{
  return(
    <h1 style={{fontSize:'75pt'}}>{props.myName}</h1>
  )}  
const InfoElements = (props) =>{
  return(
    props.infoList.map(e=> <li key={e}>{e}</li>)
  )}
  const SkillsElements =(props)=>{
    return (
      props.skillsList.map (e=> <div style={skillsStyles}>{e}</div>)
    )}
...
const FrontEndTechs = (props) =>{
  let techListItems = props.data.map(e=><img style={techLogoStyles} src={e.src} alt={e.alt}></img>)
  return(
...
const App = ()=>{
  ...
  <div style={appStyles}>
      ...
      <FrontEndTechs data={frontEndTechList}/>
      <FormDiv />
      <RandomColorBox />
