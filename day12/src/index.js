...
class App extends Component {
  ...
  validate = () => {
    ...
    if (!/^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/.test(this.state.email) && this.state.touched.email){
      errors.email = "Enter a valid email"
    }
    if (!/^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/.test(this.state.tel) && this.state.touched.tel){
      errors.tel = "Enter a valid telephone number"
    }
    let currentDate =new Date()
    let givenDate = new Date(this.state.dateOfBirth)
    if (givenDate > currentDate && this.state.touched.dateOfBirth){
      errors.dateOfBirth = "You cannot be born in the future"
    }
    if (this.state.weight < 1 && this.state.touched.weight){
      errors.weight = 'You cannot have a negative weight'
    }
        return errors
  }
  handleSubmit = (e) => {
  ...
  render() {
    ...
    const { firstName, lastName, email, tel, dateOfBirth, weight } = this.validate()
    return (
      <div className='App'>
        <h3>Add Student</h3>
